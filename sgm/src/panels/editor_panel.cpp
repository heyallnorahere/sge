...
namespace sgm {
    ...
    void editor_panel::render() {
        ...
        if (selection) {
            switch (selection->get_type()) {
            case selection_type::entity_selection:
                valid_selection = edit_entity(selection.as<entity_selection>()->get_target());
                break;
            case selection_type::asset_selection:
                valid_selection = edit_asset(selection.as<asset_selection>()->get_target());
                break;
            default:
    ...
    bool editor_panel::edit_entity(entity target) {
        ...
        auto& tag = target.get_component<tag_component>();
        ...
        draw_component<transform_component>(
            "Transform", target, [target](transform_component& transform) {
                ...
                if (ImGui::InputInt("Z layer", &transform.z_layer)) {
                    target.get_scene()->recalculate_render_order();
                }
            });

        draw_component<camera_component>("Camera", target, [](camera_component& camera) {
        ...
        draw_component<sprite_renderer_component>(
            "Sprite renderer", target, [this, target](sprite_renderer_component& component) {
        ...
        draw_component<
            rigid_body_component>("Rigid body", target, [&](rigid_body_component& component) {
            ...
            "Script", target, [this, target](script_component& component) {
                ...
                if (ImGui::InputText("Script Name", &component.class_name)) {
                    ...
                    if (valid_script) {
                        ...
                        _scene->set_script(target, _class);
                    } else {
                        _scene->set_script(target, nullptr);
                    }
                }
    ...
    bool editor_panel::edit_asset(const fs::path& path) {
        // not implemented yet
        return true;
    }
    ...
    void editor_panel::draw_property_controls(entity target) {
        auto& sc = target.get_component<script_component>();
        sc.verify_script(target);
